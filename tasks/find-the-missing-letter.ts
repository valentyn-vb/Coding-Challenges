...
export function findTheMissingLetter(arr: string[]): string {
  const registerMethod =
    arr[0] === arr[0].toUpperCase() ? "toUpperCase" : "toLowerCase";

  let missingLetter = "";

  const firstLetterIndex = alphabet.findIndex(
    (letter) => letter === arr[0].toUpperCase(),
  );
  const lastLetterIndex = alphabet.findIndex(
    (letter) => letter === arr[arr.length - 1].toUpperCase(),
  );
